...
##
# %%%%%% data for reg1 %%%%%%%%%%%%%
#full mixed model for 
Reg2<- as.numeric(Reg2)
emotion.model<- lmer(Reg2~ group + time+ (1|ID)+ (1|Age), data=edata,REML = FALSE)
# null model 
null.model<- lmer(Reg2~1 + (1|ID)+(1|Age), data=edata, REML = FALSE)
#model with  the interaction
inter.model <- lmer(Reg2~group* time+ (1|Age)+(1|ID), data = edata,REML = FALSE)
# model for the effect of group
group.model <- lmer(Reg2~ time+ (1|Age)+(1|ID), data = edata,REML = FALSE)
#model  for effect of time
time.model <- lmer(Reg2~ group+ (1|Age)+(1|ID), data = edata,REML = FALSE)
###
##
#
...
time.model.experiment<- lmer(Reg2~ time+ (1|ID)
                             ,data = group1,REML = FALSE)

null.model.experiment <- lmer(Reg2~ 1+(1|ID),
                              data = group1,REML = FALSE)
...
time.model.control<- lmer(Reg2~ time+(1|ID),
                          data = edata,REML = FALSE)

null.model.control <- lmer(Reg2~ 1+(1|ID),
                           data = edata,REML = FALSE)
